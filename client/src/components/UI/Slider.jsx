                            ...
                            <Link to={`/titles/${el?.id}`}>
                                ...
                                {el?.isSeen ? (
                                    <PlusIcon
                                        isFilled={true}
                                        fill="white"
                                        className="absolute top-1 sm:bottom-1 right-1 transition-all z-10"
                                    />
                                ) : (
                                    ""
                                )}
                                <div className="absolute bottom-0 left-0 right-0 p-1 py-4 sm:p-4 h-full flex flex-col items- justify-end bg-opacity-50 bg-sliderGrad transition-all">
                                    <h3 className="text-xs sm:text-xl overflow-hidden text-ellipsis whitespace-nowrap font-medium">
                                        {el?.title}
                                    </h3>
                                    {showDate ? (
                                        <p>
